...
export default function BlogVideoManager({ mode = "blog" }: { mode?: "blog" | "video" }) {
  ...
  const saveBlog = () => {
    if (!blog) return;
    const payload: BlogPostPayload = { ...blog, sortOrder: Number(blog.sortOrder) || 100 };
    const done = async () => { await utils.blog.adminList.invalidate(); setNotice(`「${payload.title}」を保存しました。`); setBlog(null); };
    if (blog.id) updateBlog.mutate({ id: blog.id, post: payload }, { onSuccess: done, onError: error => setNotice(error.message) });
    else createBlog.mutate(payload, { onSuccess: done, onError: error => setNotice(error.message) });
  };

  const saveVideo = () => {
    if (!video) return;
    const payload: VideoPayload = { ...video, sortOrder: Number(video.sortOrder) || 100 };
    const done = async () => { await utils.videos.adminList.invalidate(); setNotice(`「${payload.title}」を保存しました。`); setVideo(null); };
    if (video.id) updateVideo.mutate({ id: video.id, video: payload }, { onSuccess: done, onError: error => setNotice(error.message) });
    else createVideo.mutate(payload, { onSuccess: done, onError: error => setNotice(error.message) });
  };
  ...
  return <DashboardLayout><div className="mx-auto max-w-6xl pb-20">
    ...
    <div className="grid gap-7 lg:grid-cols-[.75fr_1.25fr]"><section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><div><h2 className="text-base font-bold">{isBlog ? "記事一覧" : "動画一覧"}</h2><p className="mt-1 text-xs text-slate-500">選択して編集できます。</p></div><button type="button" onClick={() => setSelected(clone(isBlog ? EMPTY_BLOG_POST : EMPTY_VIDEO))} className="inline-flex items-center gap-2 rounded-md bg-[#102845] px-3 py-2 text-xs font-bold text-white"><FilePlus2 size={15} />新規作成</button></div><div className="mt-5 space-y-2">{(isBlog ? blogLoading : videoLoading) ? <Loader2 className="animate-spin text-sky-700" /> : records.length === 0 ? <p className="rounded-lg bg-slate-50 p-4 text-sm text-slate-500">まだ登録がありません。</p> : records.map(item => <button type="button" key={item.id} onClick={() => setSelected(clone(item))} className="w-full rounded-lg border border-slate-200 p-3 text-left hover:border-sky-400 hover:bg-sky-50"><span className="block text-sm font-bold text-slate-900">{item.title}</span><span className="mt-1 flex justify-between text-xs text-slate-500"><span>/{isBlog ? "blog" : "videos"}/{item.slug}</span><span>{item.isPublished ? "公開中" : "下書き"}</span></span></button>)}</div></section>
    <section className="min-h-[520px] rounded-xl border border-slate-200 bg-slate-50 p-5 shadow-sm">{selected ? <div><div className="flex items-start justify-between border-b border-slate-200 pb-5"><div><p className="font-mono text-xs tracking-widest text-sky-700">{selected.id ? "EDIT" : "NEW"}</p><h2 className="mt-2 text-xl font-bold">{selected.id ? "編集" : "新規作成"}</h2></div><button type="button" onClick={() => setSelected(null)} className="text-xs font-bold text-slate-500">閉じる</button></div>{isBlog && blog ? <div className="mt-5 space-y-5"><div className="grid gap-4 md:grid-cols-2"><Field label="タイトル" value={blog.title} onChange={value => setBlog({ ...blog, title: value })} /><Field label="スラッグ" value={blog.slug} hint="半角英数とハイフン" onChange={value => setBlog({ ...blog, slug: value.toLowerCase().replace(/[^a-z0-9-]/g, "") })} /></div><Field label="概要" value={blog.excerpt} textarea onChange={value => setBlog({ ...blog, excerpt: value })} /><Field label="本文" value={blog.body} textarea hint="空行で段落分け" onChange={value => setBlog({ ...blog, body: value })} /><div className="grid gap-4 md:grid-cols-2"><Field label="アイキャッチ画像URL" value={blog.coverImageUrl} onChange={value => setBlog({ ...blog, coverImageUrl: value })} hint="URLまたは /manus-storage/" /><Field label="画像代替テキスト" value={blog.coverImageAlt} onChange={value => setBlog({ ...blog, coverImageAlt: value })} /><Field label="著者" value={blog.author} onChange={value => setBlog({ ...blog, author: value })} /><Field label="公開日時" value={blog.publishedAt ?? ""} type="datetime-local" onChange={value => setBlog({ ...blog, publishedAt: value ? new Date(value).toISOString() : null })} /></div><div className="grid gap-3 md:grid-cols-2"><Toggle label="公開する" checked={blog.isPublished} note="公開後は /blog/スラッグ で閲覧できます。" onChange={value => setBlog({ ...blog, isPublished: value })} /><Field label="並び順" value={String(blog.sortOrder)} onChange={value => setBlog({ ...blog, sortOrder: Number(value) || 0 })} /></div><div className="flex gap-3"><button type="button" onClick={saveBlog} disabled={busy} className="inline-flex items-center gap-2 rounded-md bg-sky-600 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60"><Save size={15} />保存</button>{blog.id && <button type="button" onClick={removeBlog} disabled={busy} className="inline-flex items-center gap-2 rounded-md border border-rose-200 px-4 py-2.5 text-sm font-bold text-rose-700"><Trash2 size={15} />削除</button>}</div></div> : video ? <div className="mt-5 space-y-5"><div className="grid gap-4 md:grid-cols-2"><Field label="タイトル" value={video.title} onChange={value => setVideo({ ...video, title: value })} /><Field label="スラッグ" value={video.slug} hint="半角英数とハイフン" onChange={value => setVideo({ ...video, slug: value.toLowerCase().replace(/[^a-z0-9-]/g, "") })} /></div><Field label="説明" value={video.description} textarea onChange={value => setVideo({ ...video, description: value })} /><div className="grid gap-4 md:grid-cols-2"><label className="block text-sm font-semibold text-slate-800">配信方式<select value={video.sourceType} onChange={event => setVideo({ ...video, sourceType: event.target.value as VideoSourceType })} className="mt-2 w-full rounded-md border border-slate-200 bg-white px-3 py-2.5 text-sm"><option value="youtube">YouTube</option><option value="vimeo">Vimeo</option><option value="direct">直接動画URL</option><option value="storage">Manusストレージ</option></select></label><Field label="動画URL" value={video.videoUrl} hint="HTTPSの公開URL" onChange={value => setVideo({ ...video, videoUrl: value })} /></div><div className="grid gap-4 md:grid-cols-2"><Field label="サムネイルURL" value={video.thumbnailUrl} onChange={value => setVideo({ ...video, thumbnailUrl: value })} /><Field label="サムネイル代替テキスト" value={video.thumbnailAlt} onChange={value => setVideo({ ...video, thumbnailAlt: value })} /><Field label="公開日時" value={video.publishedAt ?? ""} type="datetime-local" onChange={value => setVideo({ ...video, publishedAt: value ? new Date(value).toISOString() : null })} /><Field label="並び順" value={String(video.sortOrder)} onChange={value => setVideo({ ...video, sortOrder: Number(value) || 0 })} /></div><Toggle label="公開する" checked={video.isPublished} note="公開後は /videos/スラッグ で閲覧できます。" onChange={value => setVideo({ ...video, isPublished: value })} /><div className="flex gap-3"><button type="button" onClick={saveVideo} disabled={busy} className="inline-flex items-center gap-2 rounded-md bg-sky-600 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60"><Save size={15} />保存</button>{video.id && <button type="button" onClick={removeVideo} disabled={busy} className="inline-flex items-center gap-2 rounded-md border border-rose-200 px-4 py-2.5 text-sm font-bold text-rose-700"><Trash2 size={15} />削除</button>}</div></div> : null}</div> : <div className="flex min-h-[480px] items-center justify-center text-center text-sm text-slate-500"><p>左の一覧から選択するか、新規作成してください。</p></div>}</section></div>
  </div></DashboardLayout>;
}
