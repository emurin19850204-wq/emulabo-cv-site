...
export default function BlogVideoManager({ mode = "blog" }: { mode?: "blog" | "video" }) {
  ...
  const saveBlog = () => {
    ...
    const onSuccess = async () => { await utils.blog.adminList.invalidate(); setNotice(`「${payload.title}」を保存しました。`); setBlog(null); };
    if (blog.id) updateBlog.mutate({ id: blog.id, post: payload }, { onSuccess, onError: error => setNotice(error.message) });
  ...
  const saveVideo = () => {
    ...
    const onSuccess = async () => { await utils.videos.adminList.invalidate(); setNotice(`「${payload.title}」を保存しました。`); setVideo(null); };
    if (video.id) updateVideo.mutate({ id: video.id, video: payload }, { onSuccess, onError: error => setNotice(error.message) });
